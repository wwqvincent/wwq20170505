/**
 * Created by Avery on 2016/9/12.
 * Edited by Avery on 2016/9/13 1:56.
 */
...
function move(obj,json,fn){  	// 属性值可以不加px 透明度是用0-1之间的数值
    ...
    obj.timer = setInterval(function(){
        ...
        for (var attr in json){  //每个属性的遍历修改
            ...
            if(attr == "opacity"){	//透明度
                iCur = parseFloat(getStyle(obj,"opacity"))*10;
                iTarget = parseInt(json[attr]*10);
            }else{
                iCur = Math.round(parseFloat(getStyle(obj,attr)));
                iTarget = parseInt(json[attr]);
            }

            iSpeed = (iTarget - iCur)/2;//获取相对速度
            ...
            if(iCur != iTarget){
                ...
                if(attr == "opacity"){
                    ...
                    obj.style.opacity =(iCur + iSpeed)/10;
                }else{
